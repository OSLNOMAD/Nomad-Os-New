...
- **Nomad QR App** (`/ops/nomad-qr`): Internal staff-only tool for managing WiFi device credentials. Routes: `/ops/nomad-qr` (main app), `/ops/nomad-qr/print/:imei` (dedicated print page). Features: (1) **Mobile scan flow** — enter IMEI, capture sticker photo, OCR extraction of SSID/password via Tesseract.js, confirm/edit, save to DB. (2) **Desktop search & print** — search devices by IMEI or SSID, view details with password reveal (audit-logged), print 4x6 labels via dedicated print route. (3) **Permission-based access** — requires admin login plus explicit QR access grant from admin dashboard's "Nomad QR Access" tab. All actions (create, update, reveal password, print, grant/revoke access) are logged in `qr_audit_logs`. DB tables: `qr_access_grants`, `qr_device_records`, `qr_audit_logs`.
